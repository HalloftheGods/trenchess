...
const GameBoard: React.FC<GameBoardProps> = ({
  ...
  const isCellFogged = (r: number, c: number): boolean => {
    ...
    if (mode === "2p-ns") {
      return turn === "player1" ? r >= 6 : r < 6;
    } else if (mode === "2p-ew") {
      if (turn === "player3") return c >= 6;
      if (turn === "player2") return c < 6;
    } else {
      let isMyArea = false;
      if (turn === "player1") isMyArea = r < 6 && c < 6;
      if (turn === "player2") isMyArea = r < 6 && c >= 6;
      if (turn === "player3") isMyArea = r >= 6 && c < 6;
      if (turn === "player4") isMyArea = r >= 6 && c >= 6;
      return !isMyArea;
    }
    return false;
  };
