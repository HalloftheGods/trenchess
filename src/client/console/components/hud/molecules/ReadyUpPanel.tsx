...
import { PLAYER_CONFIGS, INITIAL_ARMY, PHASES } from "@constants";
import { PlayerTypeToggle } from "../atoms";
import type { BoardPiece, ArmyUnit, GameState } from "@/shared/types/game";
...
export const ReadyUpPanel: React.FC<ReadyUpPanelProps> = ({
  ...
  const isSetup = gameState === PHASES.MAIN || gameState === PHASES.GENESIS;
    ...
    <div className="relative group">
      ...
      <div
        ...
        className={`
          ...
          ${canSelect ? "cursor-pointer hover:border-slate-300 dark:hover:border-white/20 active:scale-95" : ""}
          ${
            isReady
              ? `${borderClass} ring-1 ring-white/10 shadow-[0_0_40px_rgba(var(--${cfg.color.split("-")[0]}-rgb),0.3)]`
              : isLocalTurn
                ? `${borderClass} shadow-slate-100 dark:shadow-white/5 ring-2 ring-offset-4 ring-offset-slate-900 ring-indigo-500/50 scale-[1.02] z-20`
                : `${borderMutedClass} opacity-60 grayscale-[0.5] hover:opacity-100 hover:grayscale-0`
          }
