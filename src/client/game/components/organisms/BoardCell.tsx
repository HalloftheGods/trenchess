    ...
    >
      {!fogged && terrainType === TERRAIN_TYPES.TREES && (
        <Flex
          align="center"
          justify="center"
          className={`absolute inset-0 border border-emerald-600/50 dark:border-emerald-400/50 ${hasUnit ? "bg-emerald-900 dark:bg-emerald-950" : "bg-emerald-600/10 dark:bg-emerald-400/10"}`}
          style={rotationStyle}
        >
          {!hasUnit && (
      ...
      {!fogged && terrainType === TERRAIN_TYPES.PONDS && (
        <Flex
          align="center"
          justify="center"
          className={`absolute inset-0 border border-blue-600/50 dark:border-blue-400/50 ${hasUnit ? "bg-blue-900 dark:bg-blue-950" : "bg-blue-600/10 dark:bg-blue-400/10"}`}
          style={rotationStyle}
        >
          {!hasUnit && (
      ...
      {!fogged && terrainType === TERRAIN_TYPES.RUBBLE && (
        <Flex
          align="center"
          justify="center"
          className={`absolute inset-0 border border-brand-red/50 dark:border-brand-red/50 ${hasUnit ? "bg-red-800 dark:bg-red-900" : "bg-brand-red/10 dark:bg-brand-red/10"}`}
          style={rotationStyle}
        >
          {!hasUnit && (
      ...
      {!fogged && terrainType === TERRAIN_TYPES.DESERT && (
        <Flex
          align="center"
          justify="center"
          className={`absolute inset-0 border border-amber-600/50 dark:border-amber-400/50 ${hasUnit ? "bg-amber-900/50 dark:bg-amber-950/50" : "bg-amber-600/30 dark:bg-amber-900/20"}`}
          style={rotationStyle}
        >
          {!hasUnit && (
          ...
          {(() => {
            ...
            return (
              <Flex
                ...
                `}
              >
                {getIcon(unit, "w-full h-full", piece.player !== turn)}
              </Flex>
            );
          })()}
        </Flex>
