...
const handlePlacement = (
  ...
  const isPieceInInventory =
    inventoryIndex !== -1 && inventoryIndex !== undefined;

  if (isGM) {
    // GM bypasses inventory and points
  } else if (gameState.isMercenary) {
    if (type === "king") {
  ...
  if (currentPieceAtCell) {
    if (currentPieceAtCell.player !== playerId && !isGM) return INVALID_MOVE;
