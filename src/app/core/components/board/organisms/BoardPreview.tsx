...
import { useBoardPreview } from "@hooks/interface/useBoardPreview";
import { BOARD_SIZE } from "@constants";
import { PLAYER_CONFIGS, type PieceStyle } from "@constants";
import { INITIAL_ARMY } from "@constants";
import { TERRAIN_TYPES } from "@constants";
import { getQuadrantBaseStyle } from "@/shared/utilities/game/boardStyles";
import type { GameMode, TerrainType } from "@tc.types/game";
...
const BoardPreview: React.FC<BoardPreviewProps> = ({
  ...
  const { getBorderClasses, getIcon, getTerrainAt, getPieceAt } =
    useBoardPreview({
      selectedMode,
  ...
  return (
    <div
      className={
        isMini
          ? `relative w-full h-full bg-slate-950 overflow-hidden flex items-center justify-center transition-all duration-500`
          : `board-container group ${getBorderClasses()} ${isReady ? "board-glow-ready" : ""}`
      }
    >
      {/* CRT Scanline Effect Overlay (Moved within container) */}
      {!isMini && <div className="board-crt-overlay" />}
      {!isMini && (
      ...
      <div
        className={`w-full h-full grid grid-cols-12 ${isMini ? "gap-[1px] p-1" : "gap-[2px]"} relative z-10`}
        style={{
      ...
      {!isMini && selectedMode && playerConfig && onTogglePlayerType && (
