...
import { canUnitTraverseTerrain } from "@/app/core/mechanics";
import { UNIT_DETAILS, TERRAIN_DETAILS } from "@constants";
...
import { UnitChip } from "@/shared/components/molecules/UnitChip";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-3 w-full max-w-4xl">
          ...
          {terrain.sanctuaryUnits.map((pk: string) => (
            <UnitChip
              ...
              pieceKey={pk}
              status="sanctuary"
              isActive={activeUnit === pk}
              pieceStyle={pieceStyle}
              onClick={() => onUnitSelect(pk)}
            />
          ))}
