...
const Scoreboard = ({ darkMode, pieceStyle }: ScoreboardProps) => {
  ...
  const sidebar = useMemo(
    () => (
      <div className="w-full flex flex-col gap-8 animate-in fade-in slide-in-from-right-8 duration-500">
        <BoardPreview
          selectedMode={(hoveredMatch?.mode as GameMode) || "2p-ns"}
          selectedProtocol="custom"
          darkMode={darkMode}
          pieceStyle={pieceStyle}
          customSeed={hoveredMatch?.seed}
          showTerrainIcons={true}
          labelOverride={
            hoveredMatch
              ? getModeLabel(hoveredMatch.mode)
...
export const LazyMenuScoreboard = lazy(() => import("./MenuScoreboard"));
