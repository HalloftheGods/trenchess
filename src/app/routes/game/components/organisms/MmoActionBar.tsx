...
import { Lock, LockOpen, Dices, Bomb, Pizza, Shell } from "lucide-react";
...
const MmoActionBar: React.FC<MmoActionBarProps> = ({
  ...
  const handleReset = () => {
    if (bothLocked) return;
    if (!trenchLocked) resetTerrain();
    if (!chessLocked) resetUnits();
  };
  ...
  // Build terrain items with inventory counts + selection
  ...
  const terrainItems = terrainKeys.map((tType) => {
    ...
    const count =
      (terrainInventory[turn]?.filter((u) => u === tType) || []).length || 0;
    const isActive = placementTerrain === tType;
    const isDisabled = count === 0 || placedCount >= maxPlacement;
  ...
  // Build chess unit items with inventory counts + selection
    ...
    <div className="fixed top-4 left-0 right-0 z-50 flex items-center justify-center pointer-events-none">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <ActionBarSlot disabled={bothLocked} onClick={handleReset}>
            <Bomb
              size={18}
              className={bothLocked ? "text-slate-600" : "text-red-400"}
            />
          </ActionBarSlot>
        </div>
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Pizza
                size={18}
                className={chessLocked ? "text-slate-600" : "text-orange-400"}
              />
            </ActionBarSlot>
            <ActionBarSlot
              label="Elemental"
            ...
            >
              <Shell
                size={18}
                className={trenchLocked ? "text-slate-600" : "text-teal-400"}
              />
            </ActionBarSlot>
            <ActionBarSlot
              label="Randomize"
        ...
        <div className="flex items-end gap-1.5">
          ...
          <ActionBarPalette
            title={`Chess ${16 - (inventory[turn] || []).length}/16`}
            items={chessItems}
          />
        </div>
