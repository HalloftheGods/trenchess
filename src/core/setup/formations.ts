...
export const getVanguardFormationTargets = (
  ...
  if (isNorthSouthMode) {
    ...
    let originR = 0, originC = 0, dr = 1, dc = 1;
    if (isRed) { originR = 0; originC = 0; dr = 1; dc = 1; }
    else if (isYellow) { originR = 0; originC = 11; dr = 1; dc = -1; }
    else if (isGreen) { originR = 11; originC = 0; dr = -1; dc = 1; }
    else if (isBlue) { originR = 11; originC = 11; dr = -1; dc = -1; }

    formationTargets.push({ row: originR, col: originC, type: KING });
    formationTargets.push({ row: originR + dr, col: originC + dc, type: QUEEN });
    
    formationTargets.push({ row: originR + 2*dr, col: originC, type: ROOK });
    formationTargets.push({ row: originR, col: originC + 2*dc, type: ROOK });
    
    formationTargets.push({ row: originR + 3*dr, col: originC + dc, type: BISHOP });
    formationTargets.push({ row: originR + dr, col: originC + 3*dc, type: BISHOP });
    
    formationTargets.push({ row: originR + 4*dr, col: originC + 2*dc, type: KNIGHT });
    formationTargets.push({ row: originR + 2*dr, col: originC + 4*dc, type: KNIGHT });

    // 8 Pawns in a screening arc
    for (let i = 0; i < 4; i++) {
      formationTargets.push({ row: originR + 5*dr, col: originC + i*dc, type: PAWN });
      formationTargets.push({ row: originR + i*dr, col: originC + 5*dc, type: PAWN });
    }
  }
...
export const getFortressFormationTargets = (
  ...
  if (isNorthSouth) {
    ...
    // Protective ring
    [[midRow, 4], [midRow, 5], [midRow, 6], [midRow, 7]].forEach(([r, c]) => {
      formationTargets.push({ row: r, col: c, type: ROOK });
    });
    
    // Flanks
    [[midRow, 3], [midRow, 8]].forEach(([r, c]) => {
      formationTargets.push({ row: r, col: c, type: BISHOP });
    });
    [[outerRow, 4], [outerRow, 7]].forEach(([r, c]) => {
      formationTargets.push({ row: r, col: c, type: KNIGHT });
    });

    // Picket Line (8 Pawns)
    [2, 3, 4, 5, 6, 7, 8, 9].forEach(c => {
      formationTargets.push({ row: outerRow + (isRed ? 1 : -1), col: c, type: PAWN });
    });
  } else if (isEastWest) {
    const isGreen = player === "green";
    ...
    [[4, midCol], [5, midCol], [6, midCol], [7, midCol]].forEach(([r, c]) => {
      formationTargets.push({ row: r, col: c, type: ROOK });
    });
    [[3, midCol], [8, midCol]].forEach(([r, c]) => {
      formationTargets.push({ row: r, col: c, type: BISHOP });
    });
    [[4, outerCol], [7, outerCol]].forEach(([r, c]) => {
      formationTargets.push({ row: r, col: c, type: KNIGHT });
    });
    [2, 3, 4, 5, 6, 7, 8, 9].forEach(r => {
      formationTargets.push({ row: r, col: outerCol + (isGreen ? 1 : -1), type: PAWN });
    });
  }
  return formationTargets;
};
...
export const getSkirmishFormationTargets = (
  ...
  if (isNorthSouth) {
    ...
    // Staggered Pawns
    for (let i = 0; i < 8; i++) {
      targets.push({ row: rowBase + (i % 2 === 0 ? 4 : 5) * step, col: 2 + i, type: PAWN });
    }
  }
  return targets;
};
...
export const applyClassicalFormation = (
  ...
  players: string[],
  mode: GameMode,
  formationType: "classical" | "vanguard" | "fortress" | "skirmish" = "classical",
) => {
  ...
  players.forEach((player) => {
    ...
    if (formationType === "vanguard") formationTargets = getVanguardFormationTargets(player, mode);
    else if (formationType === "fortress") formationTargets = getFortressFormationTargets(player, mode);
    else if (formationType === "skirmish") formationTargets = getSkirmishFormationTargets(player, mode);
    else formationTargets = getClassicalFormationTargets(player, mode);
