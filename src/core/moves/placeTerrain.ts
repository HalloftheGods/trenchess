...
import type { TerrainType, TrenchessState } from "@/shared/types";
...
const handlePlacement = (
  ...
  const isCompatibleWithUnit = !unitAtCell || canPlaceUnit(unitAtCell.type, type);

  if (!isCompatibleWithUnit) return INVALID_MOVE;

  const playerTerrainInventory = gameState.terrainInventory[playerId];
  const terrainIndex = playerTerrainInventory?.indexOf(type);
  const isTerrainInInventory = terrainIndex !== -1 && terrainIndex !== undefined;
  ...
  if (isQuotaReached && isReplacingFlatTerrain) {
    return INVALID_MOVE;
  }

  if (!isReplacingFlatTerrain) {
