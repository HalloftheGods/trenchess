...
describe("placeTerrain", () => {
  ...
  beforeEach(() => {
    ...
    (coreHelpers.getQuota as any).mockReturnValue(2);
    // Allow cells [0,0], [0,1], [0,2]
    (setupLogic.getPlayerCells as any).mockReturnValue([[0, 0], [0, 1], [0, 2]]);
  ...
  it("should fail if player ID cannot be resolved", () => {
    ...
    const result = placeTerrain({ G, ctx, playerID: "0" }, 0, 0, TERRAIN_TYPES.FORESTS);
    expect(result).toBe(INVALID_MOVE);
  });

  it("should fail if placing outside of player's territory", () => {
    // cell [5,5] not in mocked getPlayerCells
    const result = placeTerrain({ G, ctx, playerID: "0" }, 5, 5, TERRAIN_TYPES.FORESTS);
    expect(result).toBe(INVALID_MOVE);
  });
  ...
  describe("Placing terrain", () => {
    ...
    it("should fail if terrain is not in inventory", () => {
      const result = placeTerrain({ G, ctx, playerID: "0" }, 0, 0, TERRAIN_TYPES.MOUNTAINS);
      expect(result).toBe(INVALID_MOVE);
    });

    it("should fail if terrain quota is reached and cell is flat", () => {
      // Mock getQuota returning 1
      (coreHelpers.getQuota as any).mockReturnValue(1);
      
      // Place one terrain manually so countPlacedTerrain returns 1
      G.terrain[0][1] = TERRAIN_TYPES.FORESTS;
      
      const result = placeTerrain({ G, ctx, playerID: "0" }, 0, 0, TERRAIN_TYPES.SWAMPS);
      expect(result).toBe(INVALID_MOVE);
    });
