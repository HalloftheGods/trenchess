...
import { RouteProvider } from "@/shared/context/RouteContext";
...
const createMockGame = (overrides = {}) => {
  const activePlayers = (overrides as any).activePlayers || ["red", "yellow", "green", "blue"];
  const readyPlayers: Record<string, boolean> = {};
  const playerTypes: Record<string, "human" | "computer"> = {};
  const inventory: Record<string, string[]> = {};
  const capturedBy: Record<string, any[]> = {};
  ...
  return {
    ...
    terrain: Array(12).fill(null).map(() => Array(12).fill(TERRAIN_TYPES.FLAT)),
    inventory,
    readyPlayers,
    playerTypes,
    capturedBy,
...
describe("GamemasterView", () => {
  ...
  it("renders all 4 player panels in 4p mode", () => {
    const game = createMockGame({ mode: "4p", activePlayers: ["red", "yellow", "green", "blue"] });
    render(<GamemasterView game={game as any} />);
  ...
  it("renders only red and blue panels in 2p-ns mode", () => {
    const game = createMockGame({ mode: "2p-ns", activePlayers: ["red", "blue"] });
    render(<GamemasterView game={game as any} />);
  ...
  it("renders only green and yellow panels in 2p-ew mode", () => {
    const game = createMockGame({ mode: "2p-ew", activePlayers: ["green", "yellow"] });
    render(<GamemasterView game={game as any} />);
